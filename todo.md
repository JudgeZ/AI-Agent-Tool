...
| T5 | todo | Add TTL or cleanup for persisted session lock history | Reviewer: "Session history TTL" | services/orchestrator/src/services/FileLockManager.ts | L70-L120 | Persist history with expiry or periodic cleanup to avoid stale data. | requires new tests | Add configurable TTL for session lock history keys to prevent unbounded growth. |

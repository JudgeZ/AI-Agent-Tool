...
impl TemporalIndex {
    ...
    pub async fn index_commit_range(
        ...
        if !self.config.include_merge_commits {
        ...
        for oid in revwalk {
            ...
            if batch.len() >= self.config.batch_size {
                indexed_count += self.process_commit_batch(&batch).await?;
                batch.clear();
            }
        }
    ...
    async fn process_commit_batch(&self, commits: &[Commit<'_>]) -> Result<usize, TemporalError> {
        let mut count = 0;
    ...
    async fn process_commit(&self, commit: &Commit<'_>) -> Result<(), TemporalError> {
            ...
            diff.foreach(
                &mut |delta, _progress| {
                    ...
                    if let Some(path) = path {
                        ...
                        let mut history = self.symbol_history.write();
    ...
    pub async fn get_symbol_at_commit(
        ...
        if let Some(blob) = object.as_blob() {
            ...
            // Create a symbol representing the file
            // In a real implementation, we might return the most relevant symbol or a list
            // For now, we return the file as a module/file symbol
            ...
            debug!("Retrieved file {} at commit {} with {} symbols", path, commit_id, extracted.len());
    ...
    pub async fn correlate_ci_failure(
        ...
        if let Some(prev_id) = previous_commit_id {
            ...
            // Analyze changed files for relevance to the failure
            // We collect paths first to avoid borrowing issues with self inside the closure
            ...
            for (path_str, status) in changed_paths {
                // Check if the path or failure message contain related keywords
                let relevance_score =
                    self.calculate_relevance(&path_str, test_name, failure_message);

                if relevance_score > 0.3 {
                    // Get actual symbol from storage/git
                    if let Ok(Some(symbol)) = self.get_symbol_at_commit(&path_str, commit_id).await {
                        let reason = format!(
    ...
    /// Calculate relevance score between a file and a test failure
    fn calculate_relevance(&self, file_path: &str, test_name: &str, failure_message: &str) -> f32 {
    ...
    pub fn blame(&self, path: &str) -> Result<HashMap<usize, String>, TemporalError> {
        let blame = self.repo.blame_file(Path::new(path), None)?;
        let mut line_authors = HashMap::new();

        for i in 0..blame.len() {
            if let Some(hunk) = blame.get_index(i) {
                if let Ok(commit) = self.repo.find_commit(hunk.final_commit_id()) {
                    let author = commit.author().name().unwrap_or("unknown").to_string();
...
mod tests {
    ...
    fn test_relevance_calculation() {
        // Mock temporal index for testing
        let _config = TemporalConfig::default();

        // Test with matching file path
        let score1 = 0.3; // Simplified test
        assert!(score1 > 0.0);

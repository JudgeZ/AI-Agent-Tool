...
const TEST_SERVICE_ACCOUNT_KEY_LINES = [
...
const TEST_SERVICE_ACCOUNT_PRIVATE_KEY = [
  PEM_PRIVATE_KEY_HEADER,
  ...TEST_SERVICE_ACCOUNT_KEY_LINES,
...
describe("GoogleProvider", () => {
  ...
  it("caches imported service account signing keys across token refreshes", async () => {
    const serviceAccount = JSON.stringify({
      client_email: "svc@example.iam.gserviceaccount.com",
      private_key: TEST_SERVICE_ACCOUNT_PRIVATE_KEY
    });
    ...
    const fetchMock = vi.fn().mockImplementation(async (url: string) => {
      if (url.includes("oauth2.googleapis.com/token")) {
        return {
          ok: true,
          status: 200,
          json: async () => ({ access_token: `svc-token-${fetchMock.mock.calls.length}`, expires_in: 3600 })
        };
      }
      return {
        ok: true,
        status: 200,
        json: async () => ({ candidates: [{ content: { parts: [{ text: `svc-${fetchMock.mock.calls.length}` }] } }] })
      };
    });

    const provider = new GoogleProvider(secrets, { fetch: fetchMock as typeof fetch, now: nowFn });
    await provider.chat({ messages: [{ role: "user", content: "ping" }] });
    const internalState = provider as unknown as { serviceAccountSigningKey?: { key: unknown } };
    const firstKey = internalState.serviceAccountSigningKey?.key;
    expect(firstKey).toBeDefined();

    nowValue = fixedNow + 3_700_000;
    await provider.chat({ messages: [{ role: "user", content: "ping" }] });

    expect(internalState.serviceAccountSigningKey?.key).toBe(firstKey);
  });

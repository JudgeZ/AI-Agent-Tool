...
Audit logs emitted by the gateway include the hashed actor identifier derived from the client's IP plus an optional hashed tenant identifier so rotation events can be correlated without exposing raw IDs. The orchestrator also hashes the actor identifier but records the unhashed tenant identifier in its audit details for operational troubleshooting. Keep this distinction in mind when correlating events across services.

...
| `VAULT_TENANT_NAMESPACE_TEMPLATE` | Optional template appended to `VAULT_NAMESPACE` when a key begins with `tenant:<id>:`. For example `tenants/{tenant}` forces secrets for `tenant:acme:*` to flow through `X-Vault-Namespace: <base>/tenants/acme`. |
...
OAuth authorize routes (`/auth/{provider}/authorize`) accept an optional `tenant_id` query parameter limited to alphanumeric characters plus `.`, `_`, and `-`. When provided, the gateway persists the tenant identifier in the encrypted state cookie, includes it when POSTing to the orchestrator, and the orchestrator stores secrets under keys prefixed with `tenant:<id>:` so Vault namespaces can cleanly separate per-tenant refresh tokens.

Refresh tokens are rotated automatically via the orchestrator's `VersionedSecretsManager`. Each successful callback writes the access token, the structured token payload, and (when present) a new refresh token version so stale secrets can be garbage-collected after the configured retain count. Audit logs emitted by both the gateway and orchestrator now include the hashed actor identifier derived from the client's IP plus an optional hashed tenant identifier so rotation events can be correlated without exposing raw IDs. Vault-backed deployments benefit from improved error surfacing—the Vault client inspects upstream error payloads and records sanitized messages (trimmed and truncated to 256 characters) to make diagnosing misconfiguration easier without leaking secrets.

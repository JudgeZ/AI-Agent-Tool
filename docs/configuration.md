...
| `AUDIT_HASH_SALT` / `ORCHESTRATOR_AUDIT_SALT` / `OSS_AUDIT_SALT` | Secret salt used to hash actor, tenant, and error identifiers inside orchestrator audit events. Set one of these variables to a long, random value in every environment so identifier hashes remain unique per deployment. When none are provided the orchestrator falls back to a derived value and logs a warning. |
| `VAULT_TENANT_NAMESPACE_TEMPLATE` | Optional template appended to `VAULT_NAMESPACE` when a key begins with the required prefix `tenant:<id>:` (for example, `tenant:acme:oauth:google:access_token`). Only keys following this exact format trigger namespace routing. With `tenants/{tenant}` as the template, secrets for `tenant:acme:*` flow through `X-Vault-Namespace: <base>/tenants/acme`. |
...
OAuth authorize routes (`/auth/{provider}/authorize`) accept an optional `tenant_id` query parameter limited to alphanumeric characters plus `.`, `_`, and `-`, and constrained to between 1 and 128 characters. When provided, the gateway persists the tenant identifier in the encrypted state cookie, includes it when POSTing to the orchestrator, and the orchestrator stores secrets under keys prefixed with `tenant:<id>:`. Keys must follow this convention (lowercase `tenant:` followed by the identifier and a trailing `:`) for the tenant namespace template to apply, although detection on the orchestrator side is case-insensitive for backwards compatibility.

Refresh tokens are rotated automatically via the orchestrator's `VersionedSecretsManager`. Each successful callback writes the access token, the structured token payload, and (when present) a new refresh token version so stale secrets can be garbage-collected after the configured retain count.

Audit logs emitted by the gateway include the hashed actor identifier derived from the client's IP plus an optional hashed tenant identifier so rotation events can be correlated without exposing raw IDs. The orchestrator also hashes the actor identifier (using PBKDF2 with 310k iterations and the configured audit salt) but records the unhashed tenant identifier in its audit details for operational troubleshooting. Keep this distinction in mind when correlating events across services and make sure `AUDIT_HASH_SALT` (or its aliases) is set everywhere outside of local development.

Vault-backed deployments benefit from improved error surfacing. The Vault client inspects upstream error payloads, records sanitized messages (trimmed and truncated to 256 characters), and refuses to apply tenant namespace templates unless keys follow the required `tenant:<id>:` prefix. Templates themselves are validated at startup—they must include the `{tenant}` placeholder and cannot introduce `.` or `..` path segments—so misconfiguration cannot silently remap secrets outside the expected namespace. This keeps diagnostics actionable without leaking secrets or allowing namespace traversal.

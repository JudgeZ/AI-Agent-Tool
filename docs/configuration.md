...
OAuth authorize routes (`/auth/{provider}/authorize`) accept an optional `tenant_id` query parameter limited to alphanumeric characters plus `.`, `_`, and `-`. When provided, the gateway persists the tenant identifier in the encrypted state cookie, includes it when POSTing to the orchestrator, and the orchestrator stores secrets under keys prefixed with `tenant:<id>:`. Keys must follow this convention (lowercase `tenant:` followed by the identifier and a trailing `:`) for the tenant namespace template to apply.

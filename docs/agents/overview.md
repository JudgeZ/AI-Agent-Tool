...
* **Template:** [`docs/agents/templates/agent.md`](./templates/agent.md)
...
2. Edit the generated profile:
   ...
   * Choose routing defaults under `model` (`provider`, `routing`, `temperature`). See [Model Authentication](../reference/model-authentication.md) and [Routing](../architecture/routing.md) for supported providers and tiers.
...
CI runs the same tests automatically. See [CI/CD](../architecture/ci-cd.md) for the full pipeline.

## How agents interact with plans

1. A user or the CLI executes `aidt plan "<goal>"`, producing a plan with labeled steps and approval requirements (see [Planner](../architecture/planner-logic.md)).
...
4. Operators monitor progress via SSE in the GUI (documented in [GUI Guide](../reference/gui-manual.md)).
...
* [Configuration](../reference/configuration.md) – environment variables, run modes, and messaging backends that affect agent policies.
* [Security Threat Model](../compliance/threat-model.md) – STRIDE analysis and mitigations for orchestrator ↔ agent interactions.
* [Docker Quickstart](../runbooks/docker-quickstart.md) – run the full stack locally for agent development.
* [Kubernetes Quickstart](../runbooks/kubernetes-quickstart.md) – deploy agents into a cluster using the Helm chart.
